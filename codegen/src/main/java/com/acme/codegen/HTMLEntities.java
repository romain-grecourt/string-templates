...
/**
 * Constants for characters escape entities in HTML.
 */
public final class HTMLEntities {
    ...
    /**
     * Copied from {@code org.apache.commons.lang3.text.translate.EntityArrays}.
     */
    @SuppressWarnings({"SpellCheckingInspection", "UnnecessaryUnicodeEscape", "GrazieInspection"})
    ...
    private static final Map<Character, String> ENTITIES_MAP;

    static {
        ENTITIES_MAP = new HashMap<>();
        for (String[] entity : ENTITIES) {
            ENTITIES_MAP.put(entity[0].charAt(0), entity[1]);
        }
    }

    /**
     * Encode the HTML entity characters in the given string.
     *
    ...
    public static String encode(String raw) {
        ...
        for (int i = 0; i < raw.length(); i++) {
            char c = raw.charAt(i);
            String entity = ENTITIES_MAP.get(c);
            if (entity != null) {
                sb.append(entity);
            } else {
                sb.append(c);
